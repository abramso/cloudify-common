...
from dsl_parser.constants import PLUGIN_EXECUTOR_KEY, CENTRAL_DEPLOYMENT_AGENT
...
def _handle_plugin_after_update(ctx, plugins_list, action):
    ...
    def add_plugin_to_central_dep_agent_plugins(plugin):
        """ Adds the plugin to the dict to prevent duplicates.
        """
        key = tuple(plugin.keys())
        central_dep_agent_plugins[key] = plugin
    ...
    for node, plugin in plugins_to_handle:
        ...
        if plugin not in plugin_list:
            if plugin[PLUGIN_EXECUTOR_KEY] == CENTRAL_DEPLOYMENT_AGENT:
                add_plugin_to_central_dep_agent_plugins(plugin)
            else:
    ...
    if action == 'add':
        add_plugins_to_install(ctx, central_dep_agent_plugins.values(), seq)
    else:
        add_plugins_to_uninstall(ctx, central_dep_agent_plugins.values(), seq)
    graph.execute()
